...
// *
// */
//public class AIplayer_firstGeneration extends Player {
//
//  private int maximumWordlength;
//
//  public AIplayer_firstGeneration(String nickname, int maximumWordLength) {
//    super(nickname);
...
//   * @return
//   */
//  public ArrayList<Field[]> getValidTilePositionsForWordlength(GameBoard gb, int numOfTiles) {
//    if (numOfTiles <= 1) {
//      return null;
//    }
//    ArrayList<Field[]> results = new ArrayList<Field[]>();
...
//              }
//            }
//            // check for parallel tiles (if we are currently at leftest position)
//            if (gb.getField(i, j).getLeft() == null
//                || gb.getField(i, j).getLeft().getTop().getTile() == null) {
//              // for current.left.top no need for non-null-check
...
//                    && gb.getField(l, j).getTop().getTile() != null) {
...
//            for (int l = i - numOfTiles; l <= i + 1; l++) {
//              if (l == i + 1) {
//                results.add(singleTilesPosition);
//                singleTilesPosition = new Field[numOfTiles];
//                break;
//              } else if (gb.getField(l, j) != null && gb.getField(l, j).getTile() == null) {
//                singleTilesPosition[l - i - numOfTiles] = gb.getField(l, j);
//              } else {
//                singleTilesPosition = new Field[numOfTiles];
//                break;
//              }
//            }
//            // check for parallel tiles (if we are currently at northest position)
//            if (gb.getField(i, j).getTop() == null
//                || gb.getField(i, j).getTop().getRight().getTile() == null) {
//              // for current.top.right no need for non-null-check
//              for (int k = j; k <= j + numOfTiles; k++) {
//                if (k == j + numOfTiles) {
//                  results.add(singleTilesPosition);
//                  singleTilesPosition = new Field[numOfTiles];
//                  break;
//                } else if (gb.getField(i, k) != null && gb.getField(i, k).getTile() == null
//                    && gb.getField(i, k).getRight().getTile() != null) {
//                  singleTilesPosition[k - j] = gb.getField(i, k);
//                } else {
//                  singleTilesPosition = new Field[numOfTiles];
//                  break;
...
//              && gb.getField(i, j).getBottom().getTile() != null) {
//            // go up vertically
//            for (int k = j - numOfTiles; k <= j + 1; k++) {
//              if (k == j + 1) {
//                results.add(singleTilesPosition);
//                singleTilesPosition = new Field[numOfTiles];
//                break;
//              } else if (gb.getField(i, k) != null && gb.getField(i, k).getTile() == null) {
//                singleTilesPosition[k - j - numOfTiles] = gb.getField(i, k);
//              } else {
//                singleTilesPosition = new Field[numOfTiles];
//                break;
//              }
...
//            // check for parallel tiles (if we are currently at leftest position)
//            if (gb.getField(i, j).getLeft() == null
//                || gb.getField(i, j).getLeft().getBottom().getTile() == null) {
//              // for current.left.bottom no need for non-null-check
//              for (int l = i; l <= i + numOfTiles; l++) {
//                if (l == i + numOfTiles) {
//                  results.add(singleTilesPosition);
//                  singleTilesPosition = new Field[numOfTiles];
//                  break;
//                } else if (gb.getField(l, j) != null && gb.getField(l, j).getTile() == null
//                    && gb.getField(l, j).getBottom().getTile() != null) {
//                  singleTilesPosition[l - i] = gb.getField(l, j);
//                } else {
//                  singleTilesPosition = new Field[numOfTiles];
//                  break;
